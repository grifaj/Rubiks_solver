...
class CubeSolver:
    ...
    def getHeuristic(self, cube):
        cube_str = self.colour_independant(cube.stringify())
        if cube_str in self.heuristic:
            return self.heuristic[cube_str]
    ...
    def ida_star(self, state, g, bound, path):
        cube = RubiksCube(state=state)
        f = g + self.getHeuristic(cube)
        if f > bound:
            return False, f, path
        if cube.solved():
            return True, g, path
        min_cost = float('inf')
        for next_action in self.generate_next_states(state):
            status, cost, new_path = self.ida_star(next_action[0], g+1, bound, path + [next_action[1]])
            if status:
    ...
    def solve_cube(self,cube):
        path = [] 
        bound = self.getHeuristic(cube)
        while True:
            status, cost, path = self.ida_star(cube.stringify(), 0, bound, path)
            if status: return cost, path
            bound = cost
